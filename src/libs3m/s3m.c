...
static void hexdump16 (uint8_t* data, uint16_t length, uint16_t offset, const char* keyword)
{
    char charstr[3];
    char nullstr[3] = ".";
    char ascii[17];
    ...
    ascii[0] = '\0';
    do {
        if (ctr == 0) printf("%04X ", o);
        printf("%02X ", data[o]);
        if (data[o] > 0x1F && data[o] < 0xA0) {
            snprintf(charstr, sizeof(charstr), "%c", data[o]);
            strcat(ascii, charstr);
        } else {
            strcat(ascii, nullstr);
        }
        ctr++;
        if (ctr == 16) {
            printf("%s \n", ascii);
 ...
 * info memory befor calling this function, without any interpretation of the 
 * S3M file content.
 */
